...
function setup() {
  ...
  // Create sunlight reflections
  for (let i = 0; i < 500; i++) {
    sunlightReflections.push({
      x: random(width),
      y: random(height * 0.6, height),
      size: random(0.5, 2),
...
function drawWater() {
  ...
  // Draw sunlight reflections
  let totalOpenness = 0;
  for (let a of anemones) {
    totalOpenness += a.openness;
  }
  let avgOpenness = totalOpenness / anemones.length;
  
  // Draw bright reflections when anemones are open
  if (avgOpenness > 0.8) {
    for (let r of sunlightReflections) {
      r.x += cos(r.angle) * r.speed;
      r.y += sin(r.angle) * r.speed;
      
      if (r.x < 0 || r.x > width) r.angle = PI - r.angle;
      if (r.y < height * 0.6 || r.y > height) r.angle = -r.angle;
      
      fill(255, 255, 200, r.brightness * 0.7);
