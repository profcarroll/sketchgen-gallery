...
let time = 0;
let formationPhase = 0; // 0 = circle, 1 = square
...
function draw() {
  ...
  for (let i = 0; i < bandMembers.length; i++) {
    ...
    // Interpolate between circle and square formation
    const t = map(formationPhase, 0, 1, 0, TWO_PI);
    const targetX = member.originalX + cos(t + i) * 150;
    const targetY = member.originalY + sin(t + i) * 150;

    // Ease towards target position
    member.x += (targetX - member.x) * 0.05;
    member.y += (targetY - member.y) * 0.05;

    fill(member.color[0], member.color[1], member.color[2]);
    noStroke();
    
    // Draw band members as rectangles to form square
