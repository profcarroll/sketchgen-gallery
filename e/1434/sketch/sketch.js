...
function draw() {
  ...
  if (frameCount % 60 === 0) {
    for (let i = 0; i < min(5, connections.length); i++) {
      let c = connections[i];
      let separationVector = p5.Vector.sub(c.b.pos, c.a.pos).normalize().mult(10);
      // This is where we could visualize the separating vector
      // But for now, we just draw a line to show it's happening
    }
  }
}
