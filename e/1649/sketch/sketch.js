...
function draw() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    let pulse = sin(time * 2 + lineObj.pulsePhase) * 0.3 + 0.7; // Pulsing between 0.4 and 1.0
    
    // Calculate position
    let x = cos(lineObj.angle + time) * lineObj.radius;
    let y = sin(lineObj.angle + time) * lineObj.radius;
    
    // Draw glow trail with pulsation
    fill(lineObj.hue, 100, 100, lineObj.alpha * pulse);
    ellipse(x, y, 2, 2);
    ...
    if (i > 0) {
      ...
      stroke(lineObj.hue, 100, 100, lineObj.alpha * pulse * 0.5);
