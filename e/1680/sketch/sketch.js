...
function draw() {
  ...
  for (let i = ships.length - 1; i >= 0; i--) {
    ...
    // Create sediment trail
    if (frameCount % 2 === 0) {
      ship.sedimentTrail.push({
        x: ship.x,
        y: ship.y,
        size: random(3, 8),
        life: 1.0,
    ...
    // Limit trail length
    if (ship.sedimentTrail.length > 30) {
      ship.sedimentTrail.shift();
    }
    
    // Update sediment trail
    for (let j = ship.sedimentTrail.length - 1; j >= 0; j--) {
      let p = ship.sedimentTrail[j];
      p.life -= 0.01;
      p.age += 0.1;
      
      if (p.life <= 0) {
        ship.sedimentTrail.splice(j, 1);
      }
    }
    ...
    // Draw sediment particles from trail
    for (let p of ship.sedimentTrail) {
    ...
    if (ship.decay > 1.0 && random() < 0.005) {
      ...
      // Add more sediment particles when ship is removed
      for (let j = 0; j < 30; j++) {
        sedimentTrails.push({
          x: ship.x,
          y: ship.y,
  ...
  for (let i = sedimentTrails.length - 1; i >= 0; i--) {
    let p = sedimentTrails[i];
