...
function draw() {
  ...
  for (let ripple of waterRipples) {
    ripple.x += cos(ripple.angle) * ripple.speed;
    ripple.y += sin(ripple.angle) * ripple.speed;
    ripple.life--;
    
    if (ripple.life <= 0 || ripple.x < 0 || ripple.x > width || ripple.y < height * 0.7 || ripple.y > height) {
      ripple.x = random(width);
      ripple.y = random(height * 0.7, height);
      ripple.life = random(100, 300);
    }
