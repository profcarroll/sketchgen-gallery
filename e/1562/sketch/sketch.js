...
function draw() {
  ...
  for (let trail of trails) {
    fill(255, 255, 200, trail.life * 0.8);
    ellipse(trail.x, trail.y, trail.size);
    
    // Update trail position and life
    trail.x += trail.speedX;
    trail.y += trail.speedY;
    trail.life -= 0.5;
    
    // Reset trail if it's too old or off-screen
    if (trail.life <= 0 || trail.x < 0 || trail.x > width || trail.y < 0 || trail.y > height) {
