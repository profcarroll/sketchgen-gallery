...
function setup() {
  ...
  for (let img of photos) {
    ...
    img.prevX = img.pos.x;
    img.prevY = img.pos.y;
    img.prevAngle = img.angle;
    img.waitPhase = random(TWO_PI);
    img.movementOffset = random(TWO_PI);
  }
...
function draw() {
  ...
  for (let scrap of scraps) {
    ...
    let moveIntensity = abs(sin(time + scrap.waitPhase)) * scrap.speed * 0.5;
  ...
  for (let photo of photos) {
    ...
    let moveIntensity = abs(sin(millis() * 0.001 + photo.waitPhase)) * photo.speed * 0.3;
