...
function draw() {
  ...
  for (let i = 0; i < streams.length; i++) {
    ...
    for (let j = 0; j <= segments; j++) {
      ...
      let pulse = sin(t * 3 + time * 2); // Rhythmic pulsation along the stream
      let oscillation = sin(t * 5 + time * 4) * 0.3; // Harmonic oscillation at edges
      let widthOffset = (oscillation + 1) * s.width * 0.5 * (1 + pulse * 0.5);
      
      let x1 = cos(t + s.phase) * (s.radius + widthOffset);
      let y1 = sin(t + s.phase) * (s.radius + widthOffset);
      
      // Brighten/dim along the stream
      let brightness = 80 + 20 * pulse;
      fill(s.hue, 80, brightness, 0.7);
