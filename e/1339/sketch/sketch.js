...
function draw() {
  ...
  for (let p of particles) {
    ...
    p.update();
    let ix = floor(constrain(p.pos.x / img.width * img.width, 0, img.width - 1));
    let iy = floor(constrain(p.pos.y / img.height * img.height, 0, img.height - 1));
    let idx = (iy * img.width + ix) * 4;
    stroke(img.pixels[idx], img.pixels[idx + 1], img.pixels[idx + 2], 180);
