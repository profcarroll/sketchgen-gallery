...
function draw() {
  ...
  for (let i = 0; i < rings.length; i++) {
    ...
    const pulse = sin(frameCount * ring.speed + ring.phase) * 0.5;
    ring.radius += pulse;
    ...
    const spiral = sin(frameCount * ring.spiralSpeed) * 0.2;
    const spiralRadius = ring.radius - spiral * 20;
    ...
    for (let j = 0; j < 3; j++) {
      const circleRadius = spiralRadius + j * 8;
      strokeWeight(2);
